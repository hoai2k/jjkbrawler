...
modules are the only way to ship a dependency. **Only `billboards/src/` may
import from here**, and only via dynamic `import()` — the game's entry path
must not load ~2 MB of 3D engine for players who never pick
`?render=billboard`. `tools/check_imports.mjs` does not scan this directory;
it is third-party code, not ours to lint.
