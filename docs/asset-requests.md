...
**Current status: rounds 1–25 delivered. NO SPRITE ROUND IS OPEN.** Rounds 24
and 25 closed on 2026-08-24 with the last of a 250-file delivery: the four
throws for the whole roster, Mahoraga's walk, teeter and crouch strike, the
strike frame of both aimed diagonal attacks, and the domain-expansion seal for
the nine fighters who have a domain. Both are
[in the history](asset-requests-history.md#round-24--the-four-throws-and-mahoragas-gaits-delivered).

**Nothing on the roster is drawn by a stand-in any more.** `node
tools/check_pose_coverage.mjs` reports 34 fighters, 1 actor and **0 undrawn
...
can be read two ways rather than of a bad delivery. What fixed each one is
[pose brief §6](../sprites/docs/pose-brief.md), and the corrected pose lines are
in §4 of the same file — they are what the next round inherits.
