...
**29 images outstanding.** Every one of them is listed below, with a full URL for anything you need to look at.

- **The sprite game** — 29 images: [29 flagged in the workbench](#outstanding-by-manifest-not-by-request) as art that exists and is wrong
- **The live-3D anime path** — 0 images
- Separately, 52 poses are drawing another pose's file. Not counted above: those are substitutions somebody chose, not images anybody is owed.
...
**No open round** in [asset-requests.md](https://raw.githubusercontent.com/hoai2k/jjkbrawler/main/docs/asset-requests.md). 29 images are still outstanding for this mode — they are flagged in the workbench rather than asked for by a round, and they are in [Outstanding by manifest, not by request](#outstanding-by-manifest-not-by-request).
