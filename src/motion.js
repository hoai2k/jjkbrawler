...
export function fighterTransform(f) {
  ...
  // while the pose did: the instant the fighter moved off the lip (or crossed
  // the stillness gate adjusting their footing) the pose flipped and several
  // degrees of body rotation vanished in one frame — the most visible pop at
  // the edge. fighter.js now DECAYS teeterT instead of zeroing it, and this
  // rides that timer wherever the pose has got to, so the lean out and the
  // lean back are the same ramp. Grounded only: a fighter who walked off the
  // lip is falling, and a falling body fading out of a lean reads as glued to
  // a ledge that is no longer there.
...
export function trailStrength(f) {
  ...
  // Projection Sorcery (Naoya): the planned frames are drawn only while the
  // technique is being HELD (fighter.js `machSprint`, B down on a run). A
  // plain run is a man running and leaves nothing behind him — the strip of
  // film used to come free with any sustained sprint, which spent the read on
  // ordinary movement. Drawn as crisp stills, not a blur: drawTrail branches
  // on char.frameTrail.
  if (f.char.frameTrail) return f.machSprint ? TRAIL_STRENGTH.mach : 0;
