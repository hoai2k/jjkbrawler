// EVERY POSE, EVERY WAY, beside the drawing it came from.
//
//   node tools/pose_three_up.mjs [char] [--out debug] [--poses idle_a,guard]
//
// The pose editor's mode cycle answers "which of these is best for THIS
...
// drawing, then the per-frame matched pose, then the baseline pose for what
// the frame IS, then the pose generated from the joints, then the clip the
// game plays today, in a row.
//
...
const MODES = ['Matched', 'Baseline match', 'Generated', 'In Game'];
...
const cells = rows.map((r) => `<figure><div class="row">
  ...
  ${MODES.map((k) => `<div><img src="data:image/png;base64,${r.shots[k]}"><figcaption>${k}<br><i>${r.how[k]}</i></figcaption></div>`).join('')}
...
.row{display:grid;grid-template-columns:repeat(5,1fr);gap:6px}img{width:100%;display:block;border-radius:3px}
...
<h1 style="font:600 15px system-ui;letter-spacing:.05em">${char.toUpperCase()} — the drawing, then ${MODES.join(', ')} (${rows.length} frames)</h1>
${cells}`);
